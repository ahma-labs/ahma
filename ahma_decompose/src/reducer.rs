...
//! produce a single aggregated text result without any LLM call.
...
pub enum ReduceMode {
    ...
    Concat,
    /// Return only the first non-empty result.
    First,
}
...
mod tests {
    ...
    fn summarize_numbers_sections() {
        ...
        assert!(out.contains("Sub-task 1"));
        assert!(out.contains("Part A"));
        assert!(out.contains("Answer A"));
        assert!(out.contains("Sub-task 2"));
    }
