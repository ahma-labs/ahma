...
- **Broad reads on macOS.** To work around APFS firmlinks, the Seatbelt profile grants global file-*read* (writes stay scoped); plaintext credential directories (`~/.aws`, `~/.gnupg`, `~/.config/gcloud`, …) are then explicitly denied, but this is wider than a container's mount namespace. The login **keychain** (`~/Library/Keychains`) is *allowed* by default (read+write) so `gh` and other Keychain-backed tools work — it is encrypted at rest and secret extraction is still gated by `securityd`; set `[sandbox] allow_keychain = false` (or `--no-allow-keychain`) to block it for maximum defense-in-depth.
