...
This crate is licensed under **AGPL-3.0**. Although it depends on
...
executable is governed by **AGPL-3.0**.
...
| `ahma_decompose` | AGPL-3.0 | Decomposition runtime |
| `ahma_worker` | AGPL-3.0 | Ephemeral worker execution |
| `ahma_renewal` | AGPL-3.0 | Renewal / unattended-session controls |
| `ahma_tui` | AGPL-3.0 | Terminal UI and approval flow |

The `ahma` executable is the assembly point for these crates, so the binary is
documented and released as **AGPL-3.0**.
