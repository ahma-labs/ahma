...
impl TaskTreeOrchestrator {
    ...
    pub fn execute_node<'a>(
        ...
        async move {
            let node = tree.get_node(node_id).ok_or_else(|| anyhow!("Node not found"))?.clone();
            ...
            let result = match &node.node_type {
                TaskType::Planning => {
                    ...
                    let completion_res = tokio::time::timeout(
                        timeout,
                        self.llm.chat_completion_with_tools(messages, &[])
                    ).await;
                    ...
                    info!("Parsed {} steps for planning node {}", parsed_steps.len(), node_id.0);
                    ...
                    for step in parsed_steps {
                        let child_scopes = if let Some(ref step_scopes) = step.sandbox_scopes {
                            ...
                            let resolved_strs = resolved.iter().map(|p| p.to_string_lossy().into_owned()).collect::<Vec<_>>();
                            Some(resolved_strs)
                        } else {
                            let parent_strs = parent_scopes.iter().map(|p| p.to_string_lossy().into_owned()).collect::<Vec<_>>();
                            Some(parent_strs)
                        };
                    ...
                    while let Some(child_id) = queue.pop_front() {
                        ...
                        for retry in 0..=max_retries {
                            if retry > 0 {
                                info!("Retrying child node {} (attempt {}/{})", child_id.0, retry, max_retries);
                        ...
                        if !success {
                            info!("Child node {} failed after maximum retries. Initiating recovery/backtracking...", child_id.0);
                            ...
                            let (failed_node_desc, outcome_str) = {
                                ...
                                let outcome = failed_node.result.as_ref().map(|r| {
                                    format!(
                                        "Exit code: {:?}\nSummary: {}\nStdout: {}\nStderr: {}",
                            ...
                            let recovery_completion_res = tokio::time::timeout(
                                recovery_timeout,
                                self.llm.chat_completion_with_tools(vec![system_msg, user_msg], &[])
                            ).await;
                            ...
                            info!("Recovery decision for node {}: action={}", node_id.0, decision.action);
                            ...
                            if decision.action == "re_plan" {
                                ...
                                info!("Re-planning node {} with {} new steps", node_id.0, new_steps.len());
                                ...
                                for step in new_steps {
                                    ...
                                    let task_type = match step.r#type.as_str() {
                                        ...
                                        "planning" => TaskType::Planning,
                                        other => return Err(anyhow!("Unsupported task type returned by LLM recovery: {}", other)),
                                    };
                ...
                TaskType::ShellCommand { command } => {
                    ...
                    let working_dir = resolved_scopes.first().cloned().unwrap_or_else(|| {
                        self.adapter.sandbox().scopes().first().cloned().unwrap_or_else(|| std::env::current_dir().unwrap())
                    });

                    info!("Running shell command node {} under sandbox scopes {:?}: {:?}", node_id.0, resolved_scopes, command);
