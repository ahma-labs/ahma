...
impl FunctionHotspot {
    ...
    /// `Self::MAX_HOTSPOTS`. Issues without a function name are grouped under
...
///
/// ```
/// use ahma_simplify::models::resolve_extensions;
///
/// // Language name
...
fn language_name_to_extensions(name: &str) -> &'static [&'static str] {
    match name.to_lowercase().as_str() {
        ...
        "kotlin" | "kt" => &["kt", "kts"],
        "swift" => &["swift"],
        "objc" | "objective-c" | "objectivec" => &["m", "mm"],
...
impl FileSimplicity {
    ...
    /// Like [`Self::calculate`], but merges in supplementary metrics from an external
