...
| Automatic exclusions and codegen detection | PASS | Directory walking respects `.gitignore` and `.ignore` files (via `ignore::WalkBuilder` with `require_git(false)`); automatically excludes build caches and dependency stores (Cargo, Gradle, npm/yarn/pnpm, Pods, DerivedData, .cxx, intermediates), database migrations, test fixtures/snapshots, and minified assets; detects and excludes machine-generated code via file content headers (UniFFI, Protobuf, OpenAPI, Bindgen, `@generated`, `DO NOT EDIT`) and filename patterns (`*FFI.h`, `*FFI.kt`, `*.pb.go`, `*.pb.rs`, etc.); supports flexible `--exclude` / `-x` globs for exact filenames, relative paths, wildcards, and directory slashes |
