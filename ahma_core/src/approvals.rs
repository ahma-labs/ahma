...
///
/// Built from [`ahma_mcp::builtin_tool::BuiltinTool::name`] rather than
/// repeating the literals: those three strings used to be typed out here
/// independently of the enum they name, which is exactly the two-lists drift
/// `builtin_tool`'s own module doc warns about. Everything *not* on this
/// list previews — see [`shows_argument_preview`].
