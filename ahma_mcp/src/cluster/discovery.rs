...
//!
//! Peers advertise themselves by:
//! 1. **mDNS** (LAN): `_ahma-worker._tcp.local` service records.
//! 2. **Static config** (`~/.ahma/cluster/peers.json`): for Tailscale or
//!    pre-configured machines.
//!
//! Discovery is passive — peers are not connected to until a task is scheduled.
//! The registry is periodically refreshed from both sources.
//!
//! ## mDNS integration
//!
//! Full mDNS requires the `mdns-sd` crate.  Until it is added to the workspace,
//! the mDNS path is a stub that logs a warning and returns an empty peer list.
//! Static peers (config file) work without `mdns-sd`.
...
impl PeerInfo {
    ...
    /// Relative load score (lower is better): active_ops is a proxy.
...
impl WorkerRegistry {
    /// Create a registry with a peer TTL of `ttl` seconds.
...
mod tests {
    ...
    fn make_peer(id: &str, model: &str) -> PeerInfo {
        PeerInfo {
            id: id.to_string(),
            addr: format!("http://127.0.0.1:3000"),
    ...
    fn registry_finds_peer_by_model() {
        ...
        let llama_peers = reg.peers_for_model("llama3.2");
        assert_eq!(llama_peers.len(), 1);
    }
