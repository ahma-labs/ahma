...
//!
//! Routes sub-tasks to the best-available `ahma worker` peer by:
//!
//! 1. Filtering to peers that have the requested model.
//! 2. Picking the peer with the lowest `active_ops` (least-loaded first).
//! 3. Sending a signed task manifest to the peer's HTTP bridge `/mcp` endpoint.
//!
//! If no remote peer is available, the scheduler falls back to running the
//! sub-task locally via the local `DecomposeOrchestrator`.
...
///
/// The `signature` field is a hex-encoded HMAC-SHA256 of the canonical JSON
/// (excluding the `signature` field itself) using the cluster shared key.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TaskManifest {
    /// Unique task ID (from the parent operation ID).
    pub task_id: String,
    /// The sub-question or prompt to answer.
    pub prompt: String,
    /// Model to use on the remote worker.
    pub model: String,
    /// LLM API base URL on the remote worker (usually localhost Ollama).
    pub llm_base_url: String,
    /// Maximum tokens for the response.
    pub max_tokens: u32,
    /// Timeout for the LLM call in seconds.
    pub timeout_secs: u64,
    /// HMAC-SHA256 signature hex (computed over fields above in canonical order).
    pub signature: String,
}

impl TaskManifest {
    /// Sign a manifest with a shared key (lightweight HMAC stub).
...
impl ClusterScheduler {
    ...
    ///
    /// Returns `None` if no suitable peer is available (caller should fall
...
/// Lightweight deterministic "HMAC" stub (not cryptographically secure).
///
/// This should be replaced with a real HMAC-SHA256 implementation before
/// production use.  It is sufficient for the initial prototype where the
/// cluster runs on a trusted LAN.
...
mod tests {
    ...
    fn scheduler_returns_none_when_no_peers() {
        // No peers registered — schedule should return None synchronously.
        let reg = WorkerRegistry::new(60);
        let scheduler = ClusterScheduler::new(reg, "test-key");
        // We can't easily await in a sync test, but we can verify the registry is empty.
