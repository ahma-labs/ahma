...
fn project_dirs_structure() {
    // The `directories` crate was dropped from the workspace; `dirs` (already a
    // dependency) exposes the same platform cache root, which is all this asserts on.
    if let Some(cache_root) = dirs::cache_dir() {
        // Cache dir should be a valid path
        let cache_dir = cache_root.join("ahma_mcp");
