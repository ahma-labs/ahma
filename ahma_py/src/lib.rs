...
//!
//! ## GPL-licensed bindings
//!
//! Decompose, vault, worker, and cluster primitives are implemented in separate
//! GPL-licensed crates (`ahma_vault`, `ahma_decompose`, etc.).  Python bindings
//! for those features are planned for a future `ahma_py_gpl` wheel that will be
//! distributed separately under GPL-3.0-or-later terms.
//!
//! ## Building the wheel
//!
//! ```bash
...
pub fn create_vault_sync(slug: &str) -> anyhow::Result<std::path::PathBuf> {
    // TaskVault is still accessible via ahma_vault if that GPL crate is added.
    // This stub shows the intended interface.
    let _ = slug;
    anyhow::bail!(
        "create_vault_sync requires ahma_vault (GPL-3.0-or-later). \
         Add ahma_vault to your Cargo.toml if you accept GPL terms."
    )
}
...
mod tests {
    ...
    fn create_vault_sync_without_ahma_vault_errors() {
        ...
        assert!(result.is_err(), "expected error without ahma_vault GPL crate");
        let msg = result.unwrap_err().to_string();
        assert!(msg.contains("GPL"), "error should mention GPL");
    }
}
