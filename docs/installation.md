...
- **Pass-through defers, it doesn't approve.** When ahma isn't the control for a call — inactive, already-wrapped, the fail-open above, or deferring to a detected host sandbox (Cursor/VS Code/Docker) — the hook response leaves the client's own permission decision unset (on Claude Code: no `permissionDecision`). Your `settings.json` rules and prompts still apply exactly as if ahma weren't installed. The one exception is the sandboxed rewrite itself (`ahma hooks run-shell ...`), which ahma explicitly allows since it *is* the substitute control for that call.
