# Ahma vs Cowork: Strategy and Roadmap
...
- Mobile native execution — defer to T3.2 outcome.
...
1. **M1 — Task Vault primitive** (~2–3 weeks)
   ...
   - File: extend [docs/security-sandbox.md](security-sandbox.md) with the Task Vault section.
...
After M3 lands, the artifact channel (T2.1) is the next high-leverage step because it converts every Ahma run into something a non-developer can interact with — which is the gating capability for the consumer use cases described above.
