...
- **Live Task Tree (SPEC R24)**: The tasks pane renders a project-scoped tree — one header per attached instance (MCP client identity, transport, scope, running/queued/succeeded/failed tallies), operations beneath it, children indented under the operation or session that spawned them (`parent_id`). Space/click accordion-expands one task into its live or historic output tail; Enter opens the full-screen operation detail overlay; instance and session headers fold their subtree; `f` toggles this-project/all-projects.
