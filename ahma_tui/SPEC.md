...
- **Stoppable, visible turns (SPEC R24.10)**: Esc on an empty input or Ctrl-C cancels the running chat turn end to end; a second Ctrl-C quits. The footer names the cancel key while a turn runs and flags a turn that has gone 60 s without an event. `q` asks again while work is running. Gate keys never fire while the chat input holds text. `/compact` keeps the last 4 turns, so the model really sees less.
