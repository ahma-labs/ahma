...
pub fn spawn_tool_call_task(
    ...
    tokio::spawn(async move {
        ...
        let session_id = match mcp.session_id {
            Some(sid) => sid,
            None => {
                ...
                match client.post(&url).json(&init_body).send().await {
                    Ok(resp) => {
                            ...
                            .and_then(|v| v.to_str().ok())
                        {
                            ...
                            let _ = tx
                                .send(BridgeEvent::ToolCallFinished {
                                    id: id.clone(),
                                    result: "Error: No mcp-session-id header in response"
                                        .to_string(),
                                    failed: true,
                                })
                                .await;
                    ...
                    Err(e) => {
                        let _ = tx
                            .send(BridgeEvent::ToolCallFinished {
                                id: id.clone(),
                                result: format!("Error: Failed to initialize session: {e}"),
                                failed: true,
                            })
                            .await;
                        return;
            ...
            .await
        {
            Ok(resp) => {
                if resp.status().is_success() {
                        ...
                        Err(e) => {
                            let _ = tx
                                .send(BridgeEvent::ToolCallFinished {
                                    id: id.clone(),
                                    result: format!(
                                        "Error: Failed to parse tool response JSON: {e}"
                                    ),
                                    failed: true,
                                })
                                .await;
                    ...
                    let _ = tx
                        .send(BridgeEvent::ToolCallFinished {
                            id: id.clone(),
                            result: format!("Error: HTTP {status}: {text}"),
                            failed: true,
                        })
                        .await;
                }
            ...
            Err(e) => {
                let _ = tx
                    .send(BridgeEvent::ToolCallFinished {
                        id: id.clone(),
                        result: format!("Error: HTTP request failed: {e}"),
                        failed: true,
                    })
                    .await;
            }
