...
//!
//! Token-economy post-processing for tool output before it reaches an AI client:
//! ANSI/carriage-return stripping, consecutive-line deduplication, repeat-output
//! fingerprinting, exit-code-aware head/tail truncation and a context-window
//! pressure governor. Pure functions and plain state, no I/O — `ahma_mcp`
//! re-exports this crate as `ahma_mcp::output_optimizer`.
