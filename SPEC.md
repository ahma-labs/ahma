...
| `--lens` analysis selection | PASS | Comma-separated `complexity`/`reuse`/`dead-code`/`all` (default `all`; `dead-code` also accepts `dead_code`/`deadcode`, case-insensitive); unknown values are a hard error listing the valid options. Selecting only non-complexity lenses skips the rust-code-analysis parse entirely |
