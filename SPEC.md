...
> Pending: Developer-ID signing + notarization of release binaries (R-SIGN.1 — blocked on Apple Developer credentials; the CI signing step is scaffolded and activates when the `APPLE_DEVELOPER_ID_*` / `APPLE_NOTARY_*` repo secrets are added — see `job-release-binaries` in `build.yml`; until then the shipped macOS artifact gets an ad-hoc hardened-runtime signature at build time, matching the posture `ahma update` retrofits locally), and the Windows WDAC/SAC verification (R-SIGN.3).
